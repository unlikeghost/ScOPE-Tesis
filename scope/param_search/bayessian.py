...
from .base import ScOPEOptimizer, ParameterSpace
...
class ScOPEOptimizerBayesian(ScOPEOptimizer):
    """
    Clase para optimización bayesiana de modelos ScOPE usando Optuna.
    Hereda de ScOPEOptimizer e implementa la funcionalidad de optimización.
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            study_name: Name for the study and generated files
    ...
    def suggest_categorical_params(self, trial) -> Dict[str, Any]:
        ...
        return {
            ...
            'compression_distance_function': trial.suggest_categorical(
                'compression_distance_function', 
                self.parameter_space.compression_distance_functions
            ),
    ...
    def suggest_model_specific_params(self, trial, model_type: str) -> Dict[str, Any]:
        """Suggest model-specific parameters - FIXED: SIEMPRE generar todos los parámetros"""        
        params = {}
                
        # Parámetros OT - SIEMPRE generados
        ...
        # SIEMPRE generar ot_matching_method_name (sin None)
        available_ot_methods = [m for m in self.parameter_space.ot_matching_method_names if m is not None]
        ...
        # Parámetros PD - SIEMPRE generados
    ...
    def print_parameter_space(self):
        ...
        print(f"  • Distance functions ({len(self.parameter_space.compression_distance_functions)}): {self.parameter_space.compression_distance_functions}")
        ...
        print(f"    • Matching methods ({len(available_ot_methods)}): {available_ot_methods}")
        ...
        total_basic = (len(self.parameter_space.compressor_names) * 
                       len(self.parameter_space.compression_distance_functions) * 
        ...
        total_ot = (len(self.parameter_space.ot_use_matching_method_options) * 
                    max(1, len(available_ot_methods)))
    ...
    def optimize(self,
        ...
        Args:
            ...
            kw_samples_validation: Validation keyword samples
    ...
    def analyze_results(self):
        ...
        if not df_results.empty:
            ...
            columns_to_show = ['value', 'params_compressor_name', 'params_compression_distance_function', 
    ...
    def save_analysis_report(self, filename: Optional[str] = None):
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f"  Distance functions ({len(self.parameter_space.compression_distance_functions)}): {self.parameter_space.compression_distance_functions}\n")
            ...
            f.write(f"    Matching methods ({len(available_ot_methods)}): {available_ot_methods}\n")
            f.write(f"  ScOPE-PD:\n")
            ...
            # Parameter importance - CORREGIDO (SIN exit())
            try:
                importances = optuna.importance.get_param_importances(self.study)
    ...
    def save_top_results_csv(self, filename: Optional[str] = None, top_n: int = 10):
        ...
        # Select relevant columns - INCLUIR PARÁMETROS ESPECÍFICOS DEL MODELO
        important_cols = ['number', 'value', 'state']
        basic_param_cols = ['compressor_name', 'compression_distance_function', 'str_separator', 
                           'use_best_sigma', 'model_type']
        model_specific_cols = []
